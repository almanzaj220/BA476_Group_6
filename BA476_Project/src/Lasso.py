# We perform LassoCV and features important here
import pandas as pd
...
lambdas = lambdas = np.logspace(-6, 2, num=1000)
reg = LassoCV(alphas = lambdas, cv=5).fit(X_train, y_train)
...
message = (f'regularization parameter{l}\n' +
           f'Validation R^2: {r_2}\n' + f'Validation MAE: {MAE}\n' + f'Validation MSE: {MSE}\n' 
           + f'Test R^2: {r_2_t}\n' + f'Test MAE: {MAE_t}\n' + f'Test MSE: {MSE_t}')
print(message)

with open("results/Lasso_results.txt", "w") as file:
    file.write(message)